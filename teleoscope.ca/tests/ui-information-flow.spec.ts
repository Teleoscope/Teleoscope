...
test.describe('UI E2E: information flow with two documents', () => {
  ...
  test('validates upload, drag/read, search/highlight, groups, and note-driven ranking', async ({
    ...
    const sharedTerm = `signal${suffix}`;
    const noteTerm = `insight${suffix}`;
    const docATitle = `Information Flow Alpha ${suffix}`;
    const docBTitle = `Information Flow Beta ${suffix}`;
    ...
    // 2) Upload two docs (CSV payload shape) through API for deterministic setup.
    await apiPostJson(page, '/api/upload/csv/chunk', {
      ...
      data: {
        ...
        rows: [
          {
            values: {
              text: `alpha ${sharedTerm} planning and ${noteTerm} context`,
              title: docATitle,
              group: 'seed-alpha'
            }
          },
          {
            values: {
              text: `beta ${sharedTerm} roadmap and ${noteTerm} context`,
              title: docBTitle,
              group: 'seed-beta'
            }
          }
    ...
    const uploaded = await waitFor(async () => {
      ...
      for (const storageId of storageIds) {
        ...
        if (titles.has(docATitle) && titles.has(docBTitle)) {
          return {
            storageId,
            label: storage.label,
            docs
    ...
    const docA = uploaded.docs.find((doc) => doc.title === docATitle);
    const docB = uploaded.docs.find((doc) => doc.title === docBTitle);
    expect(docA, `Could not find uploaded document with title "${docATitle}"`).toBeTruthy();
    expect(docB, `Could not find uploaded document with title "${docBTitle}"`).toBeTruthy();
    ...
    const seededGroup = await waitFor(async () => {
      ...
      return groups.find((g) => g.label === 'seed-alpha') ?? null;
    }, 60_000);
