...
Connection (same idea as ``backend.embeddings``):
  ...
  - or ``MILVUS_URI`` — e.g. ``https://in01-....vectordb.zillizcloud.com:19536`` (Zilliz; use the exact host + port from the Connect panel)
...
Usage (repo root; ``mamba activate teleoscope`` for pymilvus et al.; ``PYTHONPATH=.`` for ``backend`` imports):
  ...
  # Zilliz (example)
  MILVUS_URI=https://....vectordb.zillizcloud.com:19536 MILVUS_TOKEN=... \\
