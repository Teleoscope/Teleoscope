...
- `install-credential-flow.md`: investigation report for `scripts/setup.sh` -> Ansible -> Docker credential flow, drift points, and remediation priorities.
- Milvus debugging: `../scripts/milvus-status.py` (collections, partitions, row counts, optional `describe_collection`); demo + Mongo alignment: `../scripts/demo-status.sh` (add `-v` or `DEMO_STATUS_VERBOSE=1` for extra detail).
