...
def make_matrix(node_type: str, oid: ObjectId):
    return

def update_matrix(oid: ObjectId, node_type: str, graph_oid: ObjectId):
    return

def get_collection(db: database.Database, node_type):
    collection_map = {
        "Group": "groups",
        "Document": "documents",
...
def make_node(db: database.Database, oid: ObjectId, node_type):
...
def make_edge(
    ...
    db.graph.update_one(
        ...
        {
            "$push": {
                f"edges.target": schemas.create_edge(target_oid, target["_id"], edge_type)
            }
        }
    )
