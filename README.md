...
By default Milvus is published on host **`19530`** (`http://localhost:19530` for host-run seed/status scripts). Set **`MILVUS_HOST_PORT=0`** in `.env` only if `19530` is already taken (Docker picks a random port; run `docker compose port milvus 19530` to see it). The **Milvus server image** in `docker-compose.yml` and **`pymilvus`** in `backend/requirements.txt` are kept on the same release line (e.g. 2.6.x). After a major Milvus bump, local volumes may need a reset (`docker compose down -v`) and a demo re-seed.
