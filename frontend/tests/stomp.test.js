...
test("Testing dummy client publish.", () => {
  expect(new DummyClient().publish({ msg: "hello" })).toStrictEqual({
    msg: "hello",
  });
});
...
test("Testing initialize_session return value.", () => {
  expect(
    initialize_session(new DummyClient(), "paul", "test", "#ffffff")
  ).toStrictEqual({
...
test("Testing add_document_to_group return value.", () => {
  expect(
    add_user_to_session(new DummyClient(), "paul", "62a24ba3a43e59841cacad9d")
  ).toStrictEqual({
...
test("Testing save_UI_state return value.", () => {
  expect(
    save_UI_state(new DummyClient(), "62a24ba3a43e59841cacad9d", {
...
test("Testing initialize_teleoscope return value.", () => {
  expect(
    initialize_teleoscope(new DummyClient(), "62a24ba3a43e59841cacad9d")
  ).toStrictEqual({
...
test("Testing save_teleoscope_state return value.", () => {
  expect(
    save_teleoscope_state(new DummyClient(), "62a24ba3a43e59841cacad9d", {
      label: "wifi",
      positive_docs: [],
      negative_docs: [],
      stateVector: [],
...
test("Testing add_group return value.", () => {
  expect(
    add_group(new DummyClient(), "wifi", "#8c564b", "62a24ba3a43e59841cacad9d")
  ).toStrictEqual({
    task: "add_group",
    args: {
      session_id: "62a24ba3a43e59841cacad9d",
...
test("Testing add_note return value.", () => {
  expect(add_note(new DummyClient(), "v20r7t")).toStrictEqual({
    task: "add_note",
    args: {
      document_id: "v20r7t",
    },
...
test("Testing update_note return value.", () => {
  expect(
    update_note(new DummyClient(), "v20r7t", "lorem ipsum dolor")
  ).toStrictEqual({
    task: "update_note",
    args: {
      document_id: "v20r7t",
...
test("Testing reorient return value.", () => {
  expect(
    reorient(new DummyClient(), "62a24ba3a43e59841cacad9d", ["v20r7t"], [])
  ).toStrictEqual({
    task: "reorient",
    args: {
