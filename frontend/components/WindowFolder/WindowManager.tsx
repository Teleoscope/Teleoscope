...
export default function WindowManager(props) {
  ...
  const collision = useAppSelector(
    (state: RootState) => state.windows.collision
  );

  const groups = windows.filter((w) => w.type == "Group");
  const dragged_item = useAppSelector(
    (state: RootState) => state.windows.dragged
  );
  ...
  const dropping = (layout, item, e) => {
    ...
    item.w = 4;
    dispatch(
      addWindow({ i: dragged_item.id, type: dragged_item.type, ...item })
    );
  };
  ...
  return (
    <ReactGridLayout
      ...
      onDrag={(layout, oldItem, newItem, placeholder, e, element) =>
        handleCollisions(layout, newItem, placeholder)
      }
      onDragStop={(layout, oldItem, newItem, placeholder, e, element) =>
        handleDragStop(layout, newItem)
      }
