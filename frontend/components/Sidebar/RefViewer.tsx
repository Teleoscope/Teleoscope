...
import { relabelSession, updateSearch } from "@/actions/windows";
...
export default function References(props) {
  ...
  const { document } = props.windata?.demo
    ...
    : swr.useSWRAbstract(
        "document",
        `document/${selected[0].id.split("%")[0]}`
      );
    ...
    <div>
      <Stack>
        ...
        >
          ...
          {referenceGroups[selectedGroup] && referenceGroups[selectedGroup][selectedGroupIndex].map(
            (reference) => (
