...
export default function DocSet(props) {
  ...
  const inArr = (arr, item) => (arr.indexOf(item) == -1 ? false : true);
        ...
        <Card className={classes.root}>
          <div className={classes.barroot}>
            ...
            >
              ...
              <Grid item xs={9}>
                <DocSetLabel label={props.docset.label} />
              </Grid>
          ...
          <CardContent className={classes.content}>
            <SearchBar queries={props.docset.queries} handleIDs={handleIDs} />
            <Typography className={classes.dividerFullWidth} variant="overline">
              Faved documents
              {favExpand ? (
                <IconButton size="small" onClick={() => setFavExpand(false)}>
                  <ExpandMoreIcon />
                </IconButton>
              ) : (
                <IconButton size="small" onClick={() => setFavExpand(true)}>
                  <ExpandLessIcon />
                </IconButton>
              )}
              <IconButton size="small" onClick={() => breakOut}>
                <ExitToAppIcon />
              </IconButton>
              <Divider />
            </Typography>
            ...
            <Typography className={classes.dividerFullWidth} variant="overline">
              Staged documents
              {docExpand ? (
                <IconButton size="small" onClick={() => setDocExpand(false)}>
                  <ExpandMoreIcon />
                </IconButton>
              ) : (
                <IconButton size="small" onClick={() => setDocExpand(true)}>
                  <ExpandLessIcon />
                </IconButton>
              )}
              <Divider />
            ...
            {docExpand ? (
              <PostList
                data={ids
                  .filter((id) => favs.indexOf(id[0]) == -1)
                  .filter((id) => hides.indexOf(id[0]) == -1)}
                handleOpenClick={handleOpenPost}
                handleCloseClick={handleClosePost}
                handleHover={handleChildHover}
                isFavList={false}
            ...
            <Typography className={classes.dividerFullWidth} variant="overline">
              Hidden documents
              {hideExpand ? (
                <IconButton size="small" onClick={() => setHideExpand(false)}>
                  <ExpandMoreIcon />
                </IconButton>
              ) : (
                <IconButton size="small" onClick={() => setHideExpand(true)}>
                  <ExpandLessIcon />
                </IconButton>
              )}
              <Divider />
